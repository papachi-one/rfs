...
public class RemoteFileSystem implements VirtualFileSystem {
	...
	public static void main(String[] args) {
		...
		if (argsList.contains("--help") || argsList.stream().noneMatch(a -> a.startsWith("--src=")) || argsList.stream().noneMatch(a -> a.startsWith("--dst=")) || argsList.stream().noneMatch(a -> a.startsWith("--url=")) || argsList.stream().noneMatch(a -> a.startsWith("--secret="))) {
			System.out.println(
                    ...
                    Example:
                    java -jar rfs.jar --src=./ --dst=Z: --url=http://localhost/rfs.php --secret=password
                    """);
			return;
		}
		...
		RemoteFileSystem rfs = new RemoteFileSystem(src, url, secret);
		DokanyFileSystem dokanyFileSystem = new DokanyFileSystem(rfs);
		...
		int mount = dokanyFileSystem.mount(205, true, builder.build().dokanOptions(), dst, "", 120, 4096, 4096);
	...
	public RemoteFileSystem(String src, String url, String secret) {
		this.src = src;
		this.url = url;
		this.secret = secret;
	...
	public List<VirtualFileSystem.FileInfo> listDirectory(String filename) throws IOException {
		try {
			...
			return httpResponse.body().map(RemoteFileSystem::getFileInfo).toList();
		} catch (Exception e) {
			throw new IOException();
		}
	}
	...
	public void setFileSize(String filename, long size) throws IOException {
		try {
			String path = src + filename;
			HttpRequest httpRequest = HttpRequest.newBuilder(uriBuilder(url, Map.of("s", secret, "p", path, "f", "sf", "s", size))).POST(HttpRequest.BodyPublishers.noBody()).build();
			HttpResponse<Void> httpResponse = httpClient.send(httpRequest, HttpResponse.BodyHandlers.discarding());
			if (httpResponse.statusCode() != 200)
				throw new IOException();
		} catch (Exception e) {
